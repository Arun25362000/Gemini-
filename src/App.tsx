...
export default function App() {
  ...
  const addAdminLoan = async () => {
    ...
    try {
      if (adminLoanAmount > financials.availableBalance) {
        notify('error', `Low balance! Available: â‚¹${financials.availableBalance.toLocaleString('en-IN')}. Requested: â‚¹${adminLoanAmount.toLocaleString('en-IN')}`);
        return;
      }
      ...
      if (targetUser.uid) {
        createNotification(
          ...
          `An admin has recorded a ${adminLoanStatus === 'approved' ? 'approved' : 'pending'} loan of â‚¹${adminLoanAmount.toLocaleString('en-IN')} for you.`, 
          'loan'
        );
      }
      ...
      if (contrib) {
        ...
        if (targetUser && (targetUser.phoneNumber || (targetUser as any).phone)) {
          ...
          if (formattedPhone) {
            ...
            const message = `Hi ${targetUser.displayName || 'Member'}, your Unnati contribution of â‚¹${contrib.amount.toLocaleString('en-IN')} for ${monthName} ${contrib.year} has been successfully verified and approved. Thank you!`;
  ...
  const handlePayLoanInstallment = (loan: Loan, month: number, year: number, principal: number, interest: number, mode: 'online' | 'cash' = 'online') => {
  ...
  const sendWhatsAppReminder = (u: UserProfile) => {
    ...
    const message = `Hi ${u.displayName || 'Member'}, this is a reminder for your Unnati contribution of â‚¹1,000 for ${format(new Date(), 'MMMM yyyy')}. Please record your payment. Thanks!`;
    const encodedMessage = encodeURIComponent(message);
    window.open(`https://wa.me/${formattedPhone}?text=${encodedMessage}`, '_blank');
  };
  ...
  const sendEmailReminder = (u: UserProfile) => {
    ...
    const body = `Hi ${u.displayName || 'Member'},\n\nThis is a reminder for your monthly Unnati contribution of â‚¹1,000 for ${format(new Date(), 'MMMM yyyy')}. Please record your payment on the app.\n\nThanks!`;
  ...
  const sendLoanWhatsAppReminder = (u: UserProfile, amount: number, month: string) => {
    ...
    const message = `Hi ${u.displayName || 'Member'}, this is a reminder for your Unnati Loan Repayment of â‚¹${amount.toLocaleString('en-IN')} for ${month}. Please pay before the 10th to avoid late fees. Thanks!`;
    const encodedMessage = encodeURIComponent(message);
    window.open(`https://wa.me/${formattedPhone}?text=${encodedMessage}`, '_blank');
  };
  ...
  const sendLoanEmailReminder = (u: UserProfile, amount: number, month: string) => {
    ...
    const body = `Hi ${u.displayName || 'Member'},\n\nThis is a reminder for your Unnati loan repayment of â‚¹${amount.toLocaleString('en-IN')} for ${month}. Please ensure the payment is made before the 10th.\n\nThanks!`;
    const mailtoUrl = `mailto:${u.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    window.location.href = mailtoUrl;
  };
  ...
  const applyLoan = async () => {
    ...
    if (loanAmount > 50000) {
      notify('error', "Maximum loan amount is â‚¹50,000");
      return;
    }
  ...
  const exportAllDataToExcel = async () => {
    ...
    const masterReport = allUsers.filter(u => u.email !== SYSTEM_ADMIN_EMAIL).map(u => {
      ...
      sanctionedLoans.forEach((l, idx) => {
        ...
        loanDetailsList.push(
          `Loan #${idx + 1}: â‚¹${loanAmt.toLocaleString('en-IN')}${dateStr ? ` (${dateStr})` : ''} - ${statusTag} [Principal Paid: â‚¹${lPrincipalPaid.toLocaleString('en-IN')}, Balance: â‚¹${lRemainingBal.toLocaleString('en-IN')}, Interest: â‚¹${lInterestPaid.toLocaleString('en-IN')}]`
        );
      });
      ...
      return {
        ...
        'Total Deposited (â‚¹)': totalDeposited,
        ...
        'Total Sanctioned Loan Amount (â‚¹)': totalSanctionedAmount,
        'Active Loan Amount (â‚¹)': totalActiveLoanAmount,
        'Closed Loan Amount (â‚¹)': totalClosedLoanAmount,
        'Loan Principal Paid (â‚¹)': totalLoanPrincipalPaid,
        'Loan Interest Paid (â‚¹)': totalLoanInterestPaid,
        'Loan Pending Principal (â‚¹)': totalPendingPrincipal,
    ...
    const loansData = loans.map((l, idx) => {
      ...
      return {
        ...
        'Requested Amount (â‚¹)': l.amount || 0,
        'Sanctioned / Approved Amount (â‚¹)': approvedAmt,
        ...
        'Pending Balance Principal (â‚¹)': remainingBal,
    ...
    const repaymentsData = loanPayments.map((p, idx) => {
      ...
      return {
        ...
        'Loan Sanctioned (â‚¹)': parentLoan ? (parentLoan.approvedAmount || parentLoan.amount || 0) : 'N/A',
        'Repayment Month': p.month ? format(new Date(p.year || 2026, p.month - 1), 'MMMM') : 'N/A',
        'Year': p.year || 'N/A',
        'Principal Paid (â‚¹)': p.amount || 0,
        'Interest Paid (â‚¹)': p.interest || 0,
        'Total Paid (â‚¹)': (p.amount || 0) + (p.interest || 0),
    ...
    const groupSummary = [
      ...
      { 'Financial Metric': 'Total Group Subscriptions / Savings (â‚¹)', 'Value': totalGroupSavings },
      { 'Financial Metric': 'Total Loans Disbursed / Sanctioned (â‚¹)', 'Value': totalSanctionedAll },
      { 'Financial Metric': 'Total Loan Principal Repaid (â‚¹)', 'Value': totalPrincipalRepaidAll },
      { 'Financial Metric': 'Total Active Loan Pending Principal (â‚¹)', 'Value': totalActivePendingPrincipal },
      { 'Financial Metric': 'Total Interest Earned from Loans (â‚¹)', 'Value': totalInterestEarnedAll },
      { 'Financial Metric': 'Available Liquid Balance in Fund (â‚¹)', 'Value': availableLiquidBalance },
  ...
  const exportUserStatementToExcel = async () => {
    ...
    if (userLoans.length > 0) {
      const userLoansData = userLoans.map((l, idx) => {
        ...
        return {
          'Loan #': idx + 1,
          'Sanctioned Amount (â‚¹)': approvedAmt,
          'Interest Rate (%)': `${l.interestRate ?? 1}%`,
          'Tenure (Months)': l.installments || 10,
          'Status': isSettled ? 'CLOSED' : (l.status || 'PENDING').toUpperCase(),
          'Principal Paid (â‚¹)': principalPaid,
          'Interest Paid (â‚¹)': interestPaid,
          'Pending Balance (â‚¹)': remainingBal,
          'Approved Date': approvedDate
        };
      });
  ...
  const exportMonthlyCollectionsExcel = async () => {
    ...
    const summarySheetData = [
      ...
      { 'Metric': 'Total Received (Grand Total)', 'Value': `â‚¹${grandTotalMonthlyReceived.toLocaleString('en-IN')}` },
      { 'Metric': 'Total Member Subscriptions/Contributions (â‚¹)', 'Value': `â‚¹${monthlyContributionTotal.toLocaleString('en-IN')}` },
      { 'Metric': 'Total Paid Subscriptions Count', 'Value': monthlyPaidContributions.length },
      { 'Metric': 'Total Loan Repayments Collected (â‚¹)', 'Value': `â‚¹${monthlyLoanTotalCollected.toLocaleString('en-IN')}` },
      { 'Metric': 'Total Loan Repayments Count', 'Value': monthlyPaidLoanPayments.length },
      { 'Metric': 'Loan Principal Collected (â‚¹)', 'Value': `â‚¹${monthlyLoanPrincipalCollected.toLocaleString('en-IN')}` },
      { 'Metric': 'Loan Interest Collected [0.5%] (â‚¹)', 'Value': `â‚¹${monthlyLoanInterestCollected.toLocaleString('en-IN')}` },
    ...
    const memberCollectionRows = monthlyPaidContributions.map((c, idx) => {
      ...
      return {
        ...
        'Contribution Amount (â‚¹)': c.amount || 0,
    ...
    const loanRepaymentRows = monthlyPaidLoanPayments.map((p, idx) => {
      ...
      return {
        ...
        'Month & Year': monthLabel,
        'Principal Amount (â‚¹)': principal,
        'Interest Amount (â‚¹)': interest,
        'Total Repayment Paid (â‚¹)': total,
    ...
    if (isAdmin && searchQuery && searchQuery.trim()) {
      ...
      items = items.filter(l => {
        ...
        const amounts = [
          ...
          `â‚¹${reqAmount.toLocaleString('en-IN')}`,
          `â‚¹${appAmount.toLocaleString('en-IN')}`,
          `â‚¹${totalPrincipalPaid.toLocaleString('en-IN')}`,
          `â‚¹${remainingPrincipal.toLocaleString('en-IN')}`,
          `â‚¹${Math.round(remainingTotal).toLocaleString('en-IN')}`,
          `â‚¹${reqAmount}`,
          `â‚¹${appAmount}`,
          `â‚¹${totalPrincipalPaid}`,
          `â‚¹${remainingPrincipal}`
        ];
    ...
    if (isAdmin && searchQuery && searchQuery.trim()) {
      ...
      items = items.filter(p => {
        ...
        const amounts = [
          ...
          `â‚¹${p.interest.toLocaleString('en-IN')}`,
  ...
  const settleLoanImmediately = async (loan: Loan) => {
    ...
    try {
      ...
      if (isActuallyAdmin) {
        ...
        createNotification(loan.userId, "Loan Settled", `Your loan of â‚¹${loan.approvedAmount?.toLocaleString('en-IN')} has been settled immediately.`, 'loan');
        ...
        if (targetUser?.phoneNumber || (targetUser as any)?.phone) {
          const formattedPhone = formatWhatsAppNumber(targetUser.phoneNumber || (targetUser as any).phone);
          if (formattedPhone) {
            const message = `*Loan Fully Settled - Unnati Finance*\n\nHi ${targetUser.displayName || 'Member'},\n\nCongratulations! Your loan of â‚¹${loan.approvedAmount?.toLocaleString('en-IN')} is now *Paid in Full*.\n\n*Settlement Details:*\n- Principal: â‚¹${settlePrincipal.toLocaleString('en-IN')}\n- Interest: â‚¹${settleInterest.toLocaleString('en-IN')}\n- Date: ${format(new Date(settleDate), 'MMM dd, yyyy')}\n\nThank you for being a responsible member!`;
            const encodedMessage = encodeURIComponent(message);
            window.open(`https://wa.me/${formattedPhone}?text=${encodedMessage}`, '_blank');
          }
        }
  ...
  const approveLoanWithMode = async () => {
    ...
    try {
      if (loan.amount > financials.availableBalance) {
        notify('error', `Low balance! Available: â‚¹${financials.availableBalance.toLocaleString('en-IN')}. Required: â‚¹${loan.amount.toLocaleString('en-IN')}`);
        return;
      }
    ...
    try {
      ...
      if (loan) {
        ...
        if (totalPrincipalPaid >= loan.approvedAmount!) {
          await updateDoc(doc(db, 'loans', loan.id!), { status: 'paid' });
          createNotification(payment.userId, "Loan Fully Paid", `Congratulations! Your loan of â‚¹${loan.approvedAmount?.toLocaleString('en-IN')} is now fully paid.`, 'loan');
        }
      }
  ...
  const submitAdminManualRepayment = async () => {
    ...
    try {
      ...
      if (loan.approvedAmount && totalPrincipalPaid >= loan.approvedAmount) {
        await updateDoc(doc(db, 'loans', loan.id!), { status: 'paid' });
        createNotification(loan.userId, "Loan Fully Paid", `Congratulations! Your loan of â‚¹${loan.approvedAmount.toLocaleString('en-IN')} is now fully paid.`, 'loan');
      }
  ...
  return (
    <>
          ...
          >
            ...
            >
              ...
              <div className="mt-8 pt-8 border-t border-gray-100 text-sm text-gray-400">
                Monthly contribution: â‚¹1,000 before 10th
              </div>
            </motion.div>
          </motion.div>
        ) : (
      ...
      <main className={cn("max-w-6xl mx-auto px-4 sm:px-6 py-8", isMobileVisual && "px-2 py-4")}>
              ...
              {isAdmin ? (
                <div>
                  ...
                  <div className="mt-0.5 text-xl sm:text-2xl font-black text-indigo-950 tracking-tight truncate">
                    â‚¹{adminCurrentMonthTotalReceived.toLocaleString('en-IN')}
                  </div>
                </div>
              ) : (
                <div>
          ...
          {!isAdmin ? (
            ...
            >
              <div>
                ...
                <div className="mt-0.5 text-xl sm:text-2xl font-black text-emerald-950 tracking-tight">
                  â‚¹{myContributions.reduce((acc, c) => acc + c.amount, 0).toLocaleString('en-IN')}
                </div>
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div className="mt-2.5 pt-2 border-t border-purple-100/90 flex items-center justify-between text-[10.5px]">
                ...
                <span className="font-bold text-purple-700 truncate ml-1">â‚¹{financials.totalCollected.toLocaleString('en-IN')}</span>
              </div>
            </motion.div>
          )}
              ...
              >
                <div>
                  ...
                  <div className="mt-0.5 text-xl sm:text-2xl font-black text-emerald-950 tracking-tight truncate">
                    â‚¹{financials.totalSavings.toLocaleString('en-IN')}
                  </div>
                </div>
                <div className="mt-2.5 pt-2 border-t border-emerald-100/90 flex items-center justify-between text-[10.5px]">
                  ...
                  <span className="font-bold text-emerald-700 truncate ml-1">â‚¹{financials.totalInterest.toLocaleString('en-IN')}</span>
                </div>
              </motion.div>
              ...
              >
                <div>
                  ...
                  <div className="mt-0.5 text-xl sm:text-2xl font-black text-amber-700 tracking-tight truncate">
                    â‚¹{financials.outstandingPrincipal.toLocaleString('en-IN')}
                  </div>
                </div>
              ...
              >
                <div>
                  ...
                  <div className="mt-0.5 text-xl sm:text-2xl font-black text-cyan-950 tracking-tight truncate">
                    â‚¹{financials.availableBalance.toLocaleString('en-IN')}
                  </div>
                </div>
        ...
        {/* Dynamic Monthly Interest Sub-Banner below Monthly Collection Header controls */}
        {activeTab === 'monthlyCollection' && (() => {
          const now = new Date();
          const currentDay = now.getDate();
          ...
          return (
            <div className="-mt-4 mb-6 flex items-center justify-between gap-3 p-3 sm:px-4.5 sm:py-2.5 bg-gradient-to-r from-teal-50/90 via-emerald-50/60 to-cyan-50/90 border border-teal-200/90 rounded-2xl shadow-xs flex-wrap">
              <div className="flex items-center gap-2.5 flex-wrap">
                <span className="inline-flex items-center justify-center w-6 h-6 rounded-xl bg-teal-600 text-white text-xs font-black shadow-2xs">
                  %
                </span>
                <span className="text-xs sm:text-sm font-semibold text-slate-800 tracking-tight">
                  {cyclePrefix}({cycleMonthLabel}) interest:<span className="font-black text-teal-700 ml-1 bg-white px-2 py-0.5 rounded-lg border border-teal-200 shadow-2xs inline-block">Rs{dynamicInterest.toLocaleString('en-IN')}</span>
                </span>
              </div>
                    ...
                    {sortedMembers.map((u, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                            <span className="text-sm font-bold text-slate-900">â‚¹{totalPaid.toLocaleString('en-IN')}</span>
                          </td>
                          <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                            <span className={cn(
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div className="bg-slate-50 p-3 rounded-2xl">
                        ...
                        <p className="font-bold text-slate-900">â‚¹{totalPaid.toLocaleString('en-IN')}</p>
                      </div>
                      <div className="bg-slate-50 p-3 rounded-2xl">
                                      ...
                                      >
                                        ...
                                        <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                                          <div className="flex flex-col items-start gap-1">
                                            <span className="text-sm font-bold text-slate-900">â‚¹{l.amount.toLocaleString('en-IN')}</span>
                          ...
                          <div className="lg:hidden space-y-4">
                            ...
                            {loanList.map((l, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <div className={cn(
                                    "absolute top-0 right-0 px-3 py-1 text-[10px] font-black rounded-bl-xl border-b border-l transition-colors shadow-xs select-none tracking-wide",
                                    isOldestPending ? "bg-indigo-600 text-white border-indigo-700" : "bg-slate-900 text-white border-slate-950"
                                  )}>
                                  ...
                                  <div className="mb-4 grid grid-cols-3 gap-2 bg-slate-50/80 p-3 rounded-2xl border border-slate-100">
                                    <div>
                                      ...
                                      <p className="text-sm font-black text-slate-900">â‚¹{l.amount.toLocaleString('en-IN')}</p>
                                    </div>
                                    <div>
                          ...
                          {filteredLoanPayments.filter(p => p.status === 'pending').map((p, idx) => {
                            ...
                            return (
                              <div key={uniqueKey} className="bg-white p-4 rounded-2xl border border-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative overflow-hidden">
                                <div className="absolute top-0 right-0 px-2.5 py-0.5 bg-slate-900 text-[10px] font-black text-white rounded-bl-lg border-b border-l border-slate-950 shadow-xs z-10 select-none tracking-wide">
                                  #{idx + 1}
                                </div>
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                                      {format(new Date(p.year, p.month - 1), 'MMMM yyyy')} â€¢ Principal: â‚¹{p.amount.toLocaleString('en-IN')} â€¢ Interest: â‚¹{p.interest.toLocaleString('en-IN')}
                                    </p>
                                  </div>
                                </div>
                                <div className="flex items-center gap-4">
                                  <div className="text-right">
                                    <p className="text-sm font-black text-slate-900">â‚¹{(p.amount + p.interest).toLocaleString('en-IN')}</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-3.5">
                          ...
                          <div className="bg-gradient-to-br from-purple-50/90 via-purple-50/40 to-white p-3.5 sm:p-4 rounded-2xl shadow-xs border-2 border-purple-200/90 hover:border-purple-400 hover:shadow-md hover:shadow-purple-100/50 transition-all flex flex-col justify-between group relative overflow-hidden">
                            <div>
                              ...
                              <div className="mt-0.5 text-xl sm:text-2xl font-black text-purple-950 tracking-tight truncate">
                                â‚¹{financials.totalLoanIssued.toLocaleString('en-IN')}
                              </div>
                            </div>
                            <div className="mt-2.5 pt-2 border-t border-purple-100/90 flex items-center justify-between text-[10.5px]">
                          ...
                          <div className="bg-gradient-to-br from-emerald-50/90 via-emerald-50/40 to-white p-3.5 sm:p-4 rounded-2xl shadow-xs border-2 border-emerald-200/90 hover:border-emerald-400 hover:shadow-md hover:shadow-emerald-100/50 transition-all flex flex-col justify-between group relative overflow-hidden">
                            <div>
                              ...
                              <div className="mt-0.5 text-xl sm:text-2xl font-black text-emerald-700 tracking-tight truncate">
                                â‚¹{financials.totalLoanPaid.toLocaleString('en-IN')}
                              </div>
                            </div>
                            <div className="mt-2.5 pt-2 border-t border-emerald-100/90 flex items-center justify-between text-[10.5px]">
                          ...
                          <div className="bg-gradient-to-br from-amber-50/90 via-amber-50/40 to-white p-3.5 sm:p-4 rounded-2xl shadow-xs border-2 border-amber-200/90 hover:border-amber-400 hover:shadow-md hover:shadow-amber-100/50 transition-all flex flex-col justify-between group relative overflow-hidden">
                            <div>
                              ...
                              <div className="mt-0.5 text-xl sm:text-2xl font-black text-amber-700 tracking-tight truncate">
                                â‚¹{financials.totalInterest.toLocaleString('en-IN')}
                              </div>
                            </div>
                            <div className="mt-2.5 pt-2 border-t border-amber-100/90 flex items-center justify-between text-[10.5px]">
                          ...
                          <div className="bg-gradient-to-br from-teal-50/90 via-cyan-50/40 to-white p-3.5 sm:p-4 rounded-2xl shadow-xs border-2 border-teal-200/90 hover:border-teal-400 hover:shadow-md hover:shadow-teal-100/50 transition-all flex flex-col justify-between group relative overflow-hidden">
                            <div>
                              ...
                              <div className="mt-0.5 text-xl sm:text-2xl font-black text-teal-950 tracking-tight truncate">
                                â‚¹{loans.filter(l => l.status === 'approved').reduce((acc, l) => {
                                  const payments = loanPayments.filter(p => p.loanId === l.id);
                                    ...
                                    return (
                                      <React.Fragment key={`repayment-loan-frag-${keyPrefix}-${l.id || 'repay'}-${idx}`}>
                                        <tr className="hover:bg-slate-50/70 transition-colors">
                                          ...
                                          <td className="px-4 py-3.5 border-r border-slate-200/60">
                                            <div>
                                              <p className="font-bold text-indigo-600 text-sm">â‚¹{remainingTotal.toLocaleString('en-IN')}</p>
                                              <p className="text-[11px] text-slate-400 font-medium">â‚¹{remainingPrincipal.toLocaleString('en-IN')} Principal</p>
                                            </div>
                                          </td>
                                          <td className="px-4 py-3.5 border-r border-slate-200/60">
                                            <div>
                                            ...
                                            <td colSpan={6} className="p-4 sm:p-6 border-t border-b border-slate-200/80">
                                              <div className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
                                                <div className="p-4 bg-slate-50/80 border-b border-slate-200/80 flex items-center justify-between">
                                                  <h4 className="font-bold text-sm text-slate-900">Repayment Schedule â€” {targetUser?.displayName || l.userEmail}</h4>
                                                        ...
                                                        <div key={`admin-loan-schedule-${l.id || 'loan'}-${idx}-${i}`} className="flex items-center justify-between p-3 bg-white rounded-xl border border-slate-100">
                                                          <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                              ...
                                                              <p className="text-[10px] text-slate-500">â‚¹{principalToDisplay.toLocaleString('en-IN')} + â‚¹{interestToDisplay.toLocaleString('en-IN')} Int.</p>
                                                            </div>
                                                          </div>
                                                          <div className="flex items-center gap-4">
                                                            ...
                                                            <span className="text-sm font-black text-slate-900 w-20 text-right">â‚¹{total.toLocaleString('en-IN')}</span>
                                                            <div className="flex items-center gap-2">
                                                              {isPaid && (
                                                                <button 
                                  ...
                                  <div className="p-5 flex flex-col gap-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50/80 rounded-2xl border border-slate-100 text-xs">
                                      <div>
                                        ...
                                        <p className="font-bold text-indigo-600 text-sm">â‚¹{remainingTotal.toLocaleString('en-IN')}</p>
                                        <p className="text-[10px] text-slate-400">â‚¹{remainingPrincipal.toLocaleString('en-IN')} Principal</p>
                                      </div>
                                      <div>
                                      ...
                                      <div className="mt-3 p-3.5 bg-white rounded-2xl border border-slate-200/80 flex flex-wrap items-center justify-between gap-2 shadow-xs">
                                        <div>
                                          <h4 className="font-bold text-xs sm:text-sm text-slate-900">
                                            Repayment Schedule â€” {targetUser?.displayName || l.userEmail}
                                          </h4>
                                        </div>
                                              ...
                                              <div key={`admin-loan-schedule-mob-${l.id || 'loan'}-${idx}-${i}`} className="p-3.5 bg-white rounded-2xl border border-slate-200/80 shadow-xs flex flex-col gap-2.5 relative overflow-hidden">
                                                ...
                                                <div className="flex items-center justify-between pr-8">
                                                  ...
                                                  <div className="text-right flex items-center gap-2">
                                                    <span className="text-xs sm:text-sm font-black text-slate-900">â‚¹{total.toLocaleString('en-IN')}</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-5 sm:p-8 bg-indigo-600 text-white rounded-[2.5rem] shadow-sm relative overflow-hidden">
                          ...
                          <div className="text-3xl sm:text-5xl font-black mb-6">â‚¹{l.approvedAmount?.toLocaleString('en-IN')}</div>
                          ...
                          <div className="grid grid-cols-2 gap-4 sm:gap-8 pt-5 sm:pt-6 border-t border-white/10">
                            ...
                            <div>
                              ...
                              <p className="text-lg sm:text-xl font-bold">â‚¹{remainingTotal.toLocaleString('en-IN')}</p>
                              <p className="text-[10px] text-indigo-200 font-bold">â‚¹{remainingPrincipal.toLocaleString('en-IN')} Principal</p>
                            </div>
                          </div>
                        </div>
                                           ...
                                           >
                                             ...
                                             <div className="flex items-center justify-between pr-8">
                                               ...
                                               <div className="text-right">
                                                 ...
                                                 <p className="font-black text-slate-950 text-sm sm:text-base">â‚¹{total.toLocaleString('en-IN')}</p>
                                               </div>
                                             </div>
                                             ...
                                             <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 p-3 bg-slate-50/80 rounded-2xl border border-slate-100 text-xs">
                                               <div>
                                                 ...
                                                 <p className="font-bold text-slate-800">â‚¹{principalToDisplay.toLocaleString('en-IN')}</p>
                                               </div>
                                               <div>
                                                 <p className="text-[10px] font-bold text-slate-400 uppercase">Interest (0.5%)</p>
                                                 <p className="font-bold text-indigo-600">â‚¹{interest.toLocaleString('en-IN')}</p>
                                               </div>
                                               <div>
                                             ...
                                             <div className="flex items-center justify-between pt-1">
                                               ...
                                               <div className="flex items-center gap-3 ml-auto sm:ml-0">
                                                 <span className="hidden sm:inline font-black text-slate-900 w-20 text-right">â‚¹{total.toLocaleString('en-IN')}</span>
                                                 {!isPaid && !isPending && (
                                                   <button 
                                                     onClick={() => {
                                                       setSelectedLoan(l);
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden transition-all">
                        <button
                          type="button"
                          onClick={() => setIsApplicationHistoryExpanded(prev => !prev)}
                          className="w-full p-5 sm:p-6 flex items-center justify-between gap-3 text-left hover:bg-slate-50/70 transition-all cursor-pointer select-none"
                        >
                          <div className="flex items-center gap-3.5">
                            <div className="w-10 h-10 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 border border-indigo-100 shadow-2xs">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                <h3 className="text-base sm:text-lg font-bold text-slate-900">Application History</h3>
                              </div>
                              <p className="text-xs text-slate-500 font-medium mt-0.5">
                                {isApplicationHistoryExpanded ? 'Click to collapse history' : 'Click to expand and view application records'}
                              </p>
                            </div>
                          </div>

                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1.5 text-slate-700 bg-slate-100 px-3 py-1.5 rounded-xl text-xs font-bold border border-slate-200/80 shadow-2xs shrink-0">
                              <span>{userApplications.length} {userApplications.length === 1 ? 'Application' : 'Applications'}</span>
                            </div>
                            <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 border border-slate-200 shrink-0">
                              {isApplicationHistoryExpanded ? (
                                <ChevronUp className="w-4 h-4" />
                              ) : (
                                <ChevronDown className="w-4 h-4" />
                              )}
                        ...
                        <AnimatePresence initial={false}>
                          {isApplicationHistoryExpanded && (
                            <motion.div
                              key="application-history-expanded"
                              initial={{ height: 0, opacity: 0 }}
                              animate={{ height: 'auto', opacity: 1 }}
                              exit={{ height: 0, opacity: 0 }}
                              transition={{ duration: 0.25, ease: 'easeInOut' }}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                          Posted by {notice.authorName} â€¢ {notice.createdAt?.toDate ? format(notice.createdAt.toDate(), 'MMM dd, yyyy') : 'Just now'}
                        </p>
                      </div>
                    </div>
                    {isAdmin && (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-indigo-50/90 via-slate-50 to-white text-slate-900 rounded-3xl p-5 sm:p-6 shadow-sm border-2 border-indigo-200/90 transition-all">
                    <div className={cn("flex flex-col lg:flex-row lg:items-center justify-between gap-4", isMonthlyCollectionSummaryExpanded && "pb-5 border-b border-indigo-100/80")}>
                      ...
                      >
                        ...
                        <div>
                          ...
                          {isMonthlyCollectionSummaryExpanded ? (
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">
                              {format(new Date(collectionYear, collectionMonth - 1, 1), "MMMM yyyy")} â€¢ â‚¹{grandTotalMonthlyReceived.toLocaleString("en-IN")} Total
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      {isMonthlyCollectionSummaryExpanded && (() => {
                        const availableYears = getAppAvailableYears();
                        const minYear = availableYears.length > 0 ? availableYears[0] : 2026;
                        const maxYear = availableYears.length > 0 ? availableYears[availableYears.length - 1] : 2028;
                        const isPrevDisabled = collectionYear <= minYear && collectionMonth <= 1;
                        const isNextDisabled = collectionYear >= maxYear && collectionMonth >= 12;
                          ...
                          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                            <div className="flex items-center bg-white rounded-2xl p-1 border border-slate-200 shadow-xs">
                              <button 
                                ...
                                onClick={() => {
                                  ...
                                  if (collectionMonth === 1) {
                                    ...
                                    setCollectionYear(prev => prev - 1);
                                  } else {
                                    setCollectionMonth(prev => prev - 1);
                                  }
                                }}
                                className={cn(
                                ...
                                title={isPrevDisabled ? "Beginning of records" : "Previous Month"}
                              >
                              ...
                              <button 
                                ...
                                title={isNextDisabled ? "End of records" : "Next Month"}
                              >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 mt-5">
                        ...
                        <div className="bg-gradient-to-br from-emerald-50/90 via-emerald-50/30 to-white border border-emerald-200/80 rounded-2xl p-4 shadow-xs relative overflow-hidden group hover:border-emerald-300 transition-all">
                          ...
                          <p className="text-2xl font-black text-emerald-700 tracking-tight">
                            â‚¹{grandTotalMonthlyReceived.toLocaleString("en-IN")}
                          </p>
                        ...
                        <div className="bg-gradient-to-br from-indigo-50/90 via-indigo-50/30 to-white border border-indigo-200/80 rounded-2xl p-4 shadow-xs relative overflow-hidden group hover:border-indigo-300 transition-all">
                          ...
                          <p className="text-2xl font-black text-indigo-950 tracking-tight">
                            â‚¹{monthlyContributionTotal.toLocaleString("en-IN")}
                          </p>
                        ...
                        <div className="bg-gradient-to-br from-purple-50/90 via-purple-50/30 to-white border border-purple-200/80 rounded-2xl p-4 shadow-xs relative overflow-hidden group hover:border-purple-300 transition-all">
                          ...
                          <p className="text-2xl font-black text-purple-950 tracking-tight">
                            â‚¹{monthlyLoanTotalCollected.toLocaleString("en-IN")}
                          </p>
                        ...
                        <div className="bg-gradient-to-br from-amber-50/90 via-amber-50/30 to-white border border-amber-200/80 rounded-2xl p-4 shadow-xs relative overflow-hidden group hover:border-amber-300 transition-all">
                          ...
                          <div className="text-base sm:text-lg font-black text-slate-900 truncate">
                            â‚¹{monthlyLoanPrincipalCollected.toLocaleString("en-IN")} <span className="text-xs font-normal text-slate-400">+</span> <span className="text-amber-700 font-bold">â‚¹{monthlyLoanInterestCollected.toLocaleString("en-IN")}</span>
                          </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">Paid for {format(new Date(collectionYear, collectionMonth - 1, 1), "MMMM yyyy")} â€¢ {monthlyPaidContributions.length} Members</p>
                          </div>
                        </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">Principal + Interest for {format(new Date(collectionYear, collectionMonth - 1, 1), "MMMM yyyy")} â€¢ {monthlyPaidLoanPayments.length} Repayments</p>
                          </div>
                        </div>
                                        ...
                                        <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                                          <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-slate-500 font-medium">
                                              â‚¹{p.principal.toLocaleString('en-IN')} (P) + â‚¹{p.interest.toLocaleString('en-IN')} (I)
                                            </span>
                                          </div>
                                        </td>
                                        <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                        ...
                        return (
                          <>
                            ...
                            >
                              ...
                              <p className={cn("text-lg font-black", paymentMethodFilter === 'all' ? "text-white" : "text-indigo-700")}>â‚¹{total.toLocaleString('en-IN')}</p>
                            </button>
                          </>
                        );
                      })()}
                        ...
                        {sortedContributions.map((c, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                                <span className="text-sm font-bold text-slate-900">â‚¹{c.amount.toLocaleString('en-IN')}</span>
                              </td>
                              <td className="px-3 sm:px-3.5 py-3 border-r border-slate-200/60">
                                <div className="flex flex-col items-start gap-1">
                                  <span className={cn(
                        ...
                        <div className="p-5 flex flex-col gap-4">
                          ...
                          <div className="grid grid-cols-2 gap-3 p-3.5 bg-slate-50/80 rounded-2xl border border-slate-100 text-xs">
                            <div>
                              ...
                              <p className="font-black text-emerald-600 text-sm sm:text-base">â‚¹{c.amount.toLocaleString('en-IN')}</p>
                            </div>
                            <div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      {activeNoticeToast.authorName} â€¢ {activeNoticeToast.createdAt?.toDate ? format(activeNoticeToast.createdAt.toDate(), 'HH:mm') : 'Just now'}
                    </span>
                    <button 
                      onClick={() => {
                ...
                {paymentModal.mode === 'online' ? (
                  <div className="w-full flex-col flex items-center space-y-3">
                    <div className="w-full bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100/50 text-center">
                      ...
                      <p className="text-2xl font-black text-slate-900">â‚¹{paymentModal.amount.toLocaleString('en-IN')}</p>
                    </div>
                    
                    <div className="w-full bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                      <p className="text-[11px] text-slate-600 font-semibold leading-relaxed">
                        Please proceed to make the bank transfer or UPI payment of <strong className="text-indigo-600 font-extrabold">â‚¹{paymentModal.amount.toLocaleString('en-IN')}</strong> using your preferred banking/UPI application.
                      </p>
                  ...
                  <div className="w-full flex-col flex items-center space-y-3">
                    ...
                    <div className="w-full bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
                      <p className="text-[11px] text-slate-600 font-semibold leading-relaxed">
                        Please handover cash of <strong className="text-amber-600 font-extrabold">â‚¹{paymentModal.amount.toLocaleString('en-IN')}</strong> to any Unnati Group Admin.
                      </p>
                      <p className="text-[10px] text-slate-500 font-bold leading-relaxed mt-2">
        ...
        <footer className="max-w-6xl mx-auto px-4 sm:px-6 py-12 text-center">
          <p className="text-sm text-slate-400">
            Rules: â‚¹1,000 contribution due before the 10th of every month.
          </p>
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-slate-600 font-medium leading-relaxed mb-8">
                Your loan application for <span className="text-indigo-600 font-bold">â‚¹{approvedLoanPopup.amount.toLocaleString('en-IN')}</span> has been approved. The amount will be disbursed soon.
              </p>
              <button 
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button
              xœì=ÛrÛÈ•ïùŠ6+‰È‘À›$­Hò*öL­«,jä©lÖëšÈ–ˆ  ´ÄÐzÉÛþÇþD~'_²çtãŽ>€I–ªS$Ðè>}î·fL^îô¥m>-ÚvtÌžŒkúÆ5§ç¡Î‚ö–éy¾û‰·:·¿cÊkd›AðÖtøÑb4m71Öº´ù1`ÞÜØe¾;›ŽùØ¸±YÈoB#pØ¥;×³Ð7§ZîÔ0m›]¸þ˜û­rd3?gvttÄÒy3ò9Æ^°ÖÅ•Áî›öØxÚïËÙ\O¬GïÍýLÌ±{mØWñ§øÇA¿ßÒ½è@¼HŽ+l›!cF¯‘_á‹	LÛ?(¼~hãsLÜ~Fùóaïb†îTýð¡ü‘¸2y|:¶¦W_.ÅÓ®F%Ó¹€MS#Rú[	äOC"9üúPèLÂ£1öÆÖ§òÑ×åïáëFˆMgW¦{î…Æ^Kù
